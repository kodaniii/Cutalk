...
{
    ui->setupUi(this);
    ui->close_lb->init("normal","hover","pressed",
...
void FriendLabel::SetText(QString text)
{
    ...
    qDebug()<< "ui->tip_lb.width()" << ui->tip_lb->width();
    qDebug()<< "ui->close_lb->width()" << ui->close_lb->width();
    qDebug()<< "textWidth" << textWidth;
    //设置宽度
    //文本标签的宽度加上关闭按钮的宽度再加上5个像素的边界（下个标签间隔）
